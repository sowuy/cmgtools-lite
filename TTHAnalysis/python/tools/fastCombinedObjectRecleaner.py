...
class fastCombinedObjectRecleaner:
    ...
    def __init__(self,label,inlabel,cleanTausWithLooseLeptons,cleanJetsWithFOTaus,doVetoZ,doVetoLMf,doVetoLMt,jetPts,btagL_thr,btagM_thr,jetCollection='Jet',isMC=True):
        ...
        self._helper_jets = CollectionSkimmer("%sSel"%self.jc+self.label, self.jc, floats=self.vars+self.vars_jets, ints=self.vars_jets_int, maxSize=100)
    ...
    def initReaders(self,tree):
        for coll in ["LepGood","Tau",self.jc]:
    ...
    def initWorkers(self):
        ...
        self._worker.setTaus(self.nTau, self.Tau_pt, self.Tau_eta, self.Tau_phi)
        self._worker.setJets(getattr(self,'n%s'%self.jc),getattr(self,'%s_pt'%self.jc),getattr(self,'%s_eta'%self.jc),getattr(self,'%s_phi'%self.jc),
                             getattr(self,'%s_btagDeepB'%self.jc)) #,getattr(self,'%s_corr'%self.jc),getattr(self,'%s_corr_JECUp'%self.jc),getattr(self,'%s_corr_JECDown'%self.jc))
