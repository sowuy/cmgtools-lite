...
class HiggsRecoTTH(Module):
    ...
    def __init__(self,label="_Recl",cut_BDT_rTT_score = 0.0, cuts_mW_had = (50.,110.), cuts_mH_vis = (90.,130.), btagDeepCSVveto = 0.2770, doSystJEC=True): #TODO update the values here
        ...
        for var in self.systsJEC: self.branches.extend(["Hreco_%s%s"%(x,self.systsJEC[var]) for x in ["minDRlj","visHmass","Wmass","lepIdx","j1Idx","j2Idx","pTHvis","matchedpartons","bothmatchedpartons","pTHgen","etaHgen","statusHgen","motherHgen"]]) # added new branches here
...
## TODO ##
        ...
        for part in genpar:
            if part.pdgId == 25:
               if part.statusFlags &(1 << statusFlagsMap['isHardProcess']):
                  pTHgen = part.p4().Pt()
                  motherHgen = part.genPartIdxMother
                  statusHgen = part.statusFlags
                  etaHgen = part.p4().Eta()
    ...
    # ------------------
        ...
        for var in self.systsJEC:
            ...
            ret["Hreco_etaHgen%s"                   %self.systsJEC[var]] = etaHgen
            ret["Hreco_motherHgen%s"                %self.systsJEC[var]] = motherHgen
            ret["Hreco_statusHgen%s"                %self.systsJEC[var]] = statusHgen
        return ret
